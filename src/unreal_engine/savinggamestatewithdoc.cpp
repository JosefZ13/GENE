			...
			if (Actor)
			{
				for (const FName& TypeTag : Actor->Tags)
				{
					...
					if (TypeTagString.StartsWith(TEXT("Type:")))
					{
						...
						ObjectJson->SetStringField(TEXT("description"), Description);
						ObjectJson->SetStringField(TEXT("type"), Type);
...
void AGameStateStoryGen::httpSendReq()
{
	...
	* - OnResponseReceieved()
	*/
	// "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";

	for (TActorIterator<AActor> ActorItr(GetWorld()); ActorItr; ++ActorItr)
	{
		AActor* FoundActor = *ActorItr;

		if (FoundActor->GetClass()->GetName() == "StoryGenVariables") 
		{
			FProperty* ApiKeyProperty = FoundActor->GetClass()->FindPropertyByName(TEXT("apiKey"));
			
			if (ApiKeyProperty)
			{
				void* ValuePtr = ApiKeyProperty->ContainerPtrToValuePtr<void>(FoundActor);
				FString* StringValue = reinterpret_cast<FString*>(ValuePtr);
				if (StringValue)
				{
					RetrievedapiKey = *StringValue;
					UE_LOG(LogTemp, Log, TEXT("Retrieved API Key: %s"), *RetrievedapiKey);
				}
			}
		}
	}
